...
const altNumbers = (numArray) => {
    ...
    // Split the array into two arrays, postive and negative
    ...
    // Zipper the two arrays back together taking the various respective lengths into account
    if (neg.length < pos.length) {
        for (let i = 0; i < neg.length; i++) {
            newArray.push(pos[i])
            newArray.push(neg[i])
        }
        newArray.push(pos[neg.length])
    } else {
